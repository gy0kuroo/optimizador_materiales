...
def generar_pdf_presupuesto(presupuesto):
    ...
    c.drawCentredString(width / 2, 30, f"Generado el {timezone.now().strftime('%d/%m/%Y %H:%M')} - OptiCut")
...
def generar_pdf_resumen_desperdicio(optimizaciones, estadisticas, periodo='todos'):
    ...
    c.setFont("Helvetica-Bold", 14)
    c.setFillColorRGB(0, 0, 0)
    c.drawCentredString(width / 2, height - 70, "Reporte de Eficiencia - OptiCut")
